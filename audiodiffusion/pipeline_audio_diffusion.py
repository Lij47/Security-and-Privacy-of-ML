...
class AudioDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        image_tensors: torch.Tensor = None,
        slice: int = 0,
        start_step: int = 0,
        steps: int = None,
        ...
        if audio_file is not None or raw_audio is not None:
            ...
            images[:] = self.scheduler.add_noise(input_images, noise, self.scheduler.timesteps[start_step - 1])
        ...
        # bool_mask = torch.tensor(bool_mask, dtype=torch.bool).to(self.device)
        if image_tensors is not None:
            input_images = image_tensors
            input_images = (input_images / 255) * 2 - 1
            if self.vqvae is not None:
                input_images = self.vqvae.encode(torch.unsqueeze(input_images, 0)).latent_dist.sample(
                    generator=generator
                )[0]
        ...
        for step, t in enumerate(self.scheduler.timesteps[start_step:]):
            ...
            # images[:, :, :, ~bool_mask] = mask[:, step, :, ~bool_mask]
            # images[:, :, :, bool_mask] = mask[:, step, :, bool_mask]
        ...
        if self.vqvae is not None:
            ...
            # mask = 1 / 0.18215 * mask
            # mask = self.vqvae.decode(mask)["sample"]
        ...
        images = (images / 2 + 0.5).clamp(0, 1)
        images = (images * 255).round().type(torch.uint8)
        # images = list(
        #     map(lambda _: Image.fromarray(_[:, :, 0]), images)
        #     if images.shape[3] == 1
        #     else map(lambda _: Image.fromarray(_, mode="RGB").convert("L"), images)
